...
async function main() {
    ...
    const adminPasswordHash = await bcryptjs_1.default.hash('Admin123!', env_1.env.BCRYPT_ROUNDS);
    ...
    const amPasswordHash = await bcryptjs_1.default.hash('Manager123!', env_1.env.BCRYPT_ROUNDS);
    const amUser = await prisma.user.create({
        data: {
            email: 'am@mdmc.fr',
            firstName: 'Account',
            lastName: 'Manager',
            role: client_1.UserRole.ACCOUNT_MANAGER,
    ...
    const clientPasswordHash = await bcryptjs_1.default.hash('Client123!', env_1.env.BCRYPT_ROUNDS);
    ...
    const clientAccount1 = await prisma.clientAccount.create({
        data: {
            ...
            linkStatus: client_1.LinkStatus.LINKED,
